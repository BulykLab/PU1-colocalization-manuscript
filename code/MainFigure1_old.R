...
# gkmsvm
p_1c <- ggplot(pu1_gkmsvm, aes(x=REF_gkmsvm-ALT_gkmsvm, y=beta, color=-log10(p))) +
  ...
  theme(axis.title.x = element_text(size=18), axis.title.y = element_text(size=18),
        ...
        legend.background = element_rect(fill = "white", color = "black"),
        aspect.ratio=1) +
  geom_point(data=pu1_gkmsvm %>% filter(V4=='PU1_21900'), aes(x=REF_gkmsvm-ALT_gkmsvm, y=beta), shape=23, size = 3, fill='purple') +
  geom_point(data=pu1_gkmsvm %>% filter(V4=='PU1_53081'), aes(x=REF_gkmsvm-ALT_gkmsvm, y=beta), shape=23, size = 3, fill='purple') +
  geom_point(data=pu1_gkmsvm %>% filter(V4=='PU1_70939'), aes(x=REF_gkmsvm-ALT_gkmsvm, y=beta), shape=23, size = 3, fill='purple')
