...
## Fig S1 SPI1 (PU.1) expression across blood cell types
...
p_spi1_RNA <- SPI1_RNA %>% mutate(celltype = factor(celltype, levels=c("HSC", "MPP", "LMPP", "CMP", "GMP", "MEP", "CLP", "Mono", "B", "NK", "CD4T", "CD8T","Ery"))) %>%
  ...
  labs(y=expression(paste(italic("SPI1")," count per million")), x = "Cell type") +
