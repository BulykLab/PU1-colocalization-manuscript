...
#tag_enrich$qval <- qvalue(tag_enrich$P)$qvalues



p_2a <- tag_enrich %>% mutate(Category = factor(Category, levels=c("Granulocyte", "Monocyte", "Lymphocyte", "MatureRed", "ImmatureRed","Platelet", "Others"))) %>%
  ...
  scale_color_manual(values=c('#F98400', '#351C75', '#40A2EB', '#FFB6C1', '#A61C00', '#008000', "darkgray"), name="Trait group") +
  xlab("Fold enrichment") + ylab(expression(paste(-log[10],"(",italic(p)[adjusted],")"))) +
  ggrepel::geom_text_repel(aes(label=label), min.segment.length = 0, box.padding = 0.3, seed=1, max.overlaps =50) + # , ylim=c(1.4,2.5)
...
#### Figure 2c - Colocalized loci PU.1 motif altering variants bar plot ####
...
p_2c <- motif_altered %>% mutate(type = factor(type, levels=c("SNP", "Indel", "CNV", "Multi", "Unk"))) %>%
  ...
  theme_classic() +
  geom_col() + geom_text(aes(label = number), position = position_stack(vjust = 0.5), color="black", size=5) +
  theme(axis.title.x = element_text(size=18), axis.title.y = element_text(size=18),
        axis.text.x = element_text(size=14), axis.text.y = element_text(size=14),
